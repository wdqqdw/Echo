...
class FSDPSFTTrainer:
    ...
    def _build_model_optimizer(self):
        ...
        model_path = expand_model_path(self.config.model.partial_pretrain)
...
def expand_model_path(path):
    """将模型路径转换为绝对路径，并处理特殊符号（如~）"""
    # 处理以~开头的路径
    if path.startswith("~"):
...
def main(config):
    ...
    # 处理模型路径
    model_path = expand_model_path(config.model.partial_pretrain)
    local_model_path = copy_to_local(src=model_path, verbose=True)
    
    # 添加更详细的调试信息
    print(f"\n===== 模型路径调试信息 =====")
    print(f"config.model.partial_pretrain: {config.model.partial_pretrain}")
    print(f"expand_model_path 后: {model_path}")
    print(f"copy_to_local 后: {local_model_path}")
    ...
    if not os.path.exists(local_model_path):
        ...
        if os.path.exists(parent_dir):
            print(f"父目录权限: {os.access(parent_dir, os.R_OK | os.X_OK)}")
    
    if os.path.exists(local_model_path):
        print(f"路径内容: {os.listdir(local_model_path)}")
    else:
        print(f"搜索可能的模型路径:")
        possible_paths = glob.glob("/mnt/bn/chuwei-nas-hl/users/jinhaopeng/experiments/open_verl/model/*")
    ...
    print(f"trust_remote_code: {config.model.trust_remote_code}")
    
    # 临时修改：直接从已知路径加载tokenizer进行测试
    # tokenizer = hf_tokenizer(local_model_path, trust_remote_code=config.model.trust_remote_code)
    
    # 测试用：直接指定正确路径
    test_model_path = "/mnt/bn/chuwei-nas-hl/users/jinhaopeng/experiments/open_verl/model/Qwen3-1.7B"
    print(f"测试用模型路径: {test_model_path}")
    print(f"测试用路径是否存在: {os.path.exists(test_model_path)}")
    
    tokenizer = hf_tokenizer(test_model_path, trust_remote_code=config.model.trust_remote_code)
