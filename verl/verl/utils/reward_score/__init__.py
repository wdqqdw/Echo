...
def default_compute_score(data_source, solution_str, ground_truth, extra_info=None, sandbox_fusion_url=None, concurrent_semaphore=None, **kwargs):
    ...
    elif data_source in ["lighteval/MATH", "DigitalLearningGmbH/MATH-lighteval", "DeepMath-103K"]:
        from . import math
    ...
    elif data_source in ["linrany/deepscaler_only_num"]:
    ...
    elif data_source in ["policy_logic"]:
        from . import policy
        res = policy.compute_score(solution_str, ground_truth)
    elif data_source in ["policy_test"]:
        from . import policy_test
        res = policy_test.compute_score(solution_str, ground_truth)
    elif data_source in ["AVQA"]:
        from . import avqa
        assert "id" in kwargs
        assert "data_item" in kwargs
        assert "tokenizer" in kwargs
        res = avqa.compute_score(solution_str, ground_truth, kwargs["id"], kwargs["data_item"], kwargs["tokenizer"])
    else:
