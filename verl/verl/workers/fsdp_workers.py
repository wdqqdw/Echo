...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(
        ...
        from transformers import AutoConfig, AutoModelForCausalLM, AutoModelForVision2Seq, Qwen2AudioForConditionalGeneration, Qwen2_5OmniForConditionalGeneration, Qwen2_5OmniProcessor, Qwen2_5OmniThinkerForConditionalGeneration
        ...
        with init_context(), warnings.catch_warnings():
            ...
            actor_module.disable_talker()
            def forward(self, *args, **kwargs):
                return self.thinker.forward(*args, **kwargs)
            actor_module.forward = forward.__get__(actor_module)

            freeze_audio_encoder = self.config.model.freeze_audio_encoder
            if freeze_audio_encoder and hasattr(actor_module, 'audio_tower'):
                    actor_module.audio_tower.requires_grad_(False)
                    print("********* freeze AudioEncoder *********")
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        # }
        meta_info = {
            "eos_token_id": self.generation_config.eos_token_id if self.generation_config.eos_token_id is not None else self.tokenizer.eos_token_id,
